# 第 2 章：GPU 性能分析的证据层次与 Roofline 模型
...
在 Python 中调用 CUDA op，CPU 往往只是把工作提交到 GPU stream，然后继续执行。kernel launch 返回不代表设备执行已经完成，因此下面这种计时可能只测到 launch 开销。[CUDA Best Practices Guide 的 timing 章节](https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/#timing)区分 CPU timer、CUDA event 和必要同步的测量边界。
...
## 2.7 Roofline 模型与算术强度

Roofline 模型用峰值计算吞吐与内存带宽两个硬件上界描述 kernel 性能；其原始模型见 [Williams 等人的论文](https://dl.acm.org/doi/10.1145/1498765.1498785)。[Nsight Compute Profiling Guide](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#roofline-charts)进一步说明了如何用算术强度和 achieved performance 解释 GPU kernel 的 Roofline 位置：
...
### 2.8.3 Memory-bound Kernel 的优化空间
...
## 2.9 CPU/GPU Timeline 的因果关系

[Nsight Systems](https://docs.nvidia.com/nsight-systems/)把 host thread、CUDA API、stream、kernel 和数据传输放在同一时间轴上。它用于判断等待发生在哪个执行层次，而不是替代单 kernel 的指令与内存指标。
...
### 2.11.1 使用 Profiler 运行时间代替正常运行时间
...
- Williams、Waterman 与 Patterson：[Roofline: An Insightful Visual Performance Model for Multicore Architectures](https://dl.acm.org/doi/10.1145/1498765.1498785)
- PyTorch：[Profiler](https://pytorch.org/docs/stable/profiler.html)
- 课程工程：[`torch_profiler.py`](../../../scripts/profile/torch_profiler.py) 与 [`week02_profile_template.md`](../../reports/week02_profile_template.md)
