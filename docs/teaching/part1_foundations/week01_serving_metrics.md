# 第 1 章：大语言模型在线推理的请求流程与性能指标

## 1.1 从请求输入到逐 token 输出

假设用户向服务端发送了一段 1,000 token 的 prompt，并要求最多生成 100 token。

在线接口接收一次请求，但 decoder-only 自回归模型不会在一次前向计算中同时确定后续 100 个 token。后一个 token 的条件分布依赖前一个 token 的实际采样结果，因此服务端需要依次完成：
...
由计算复用、状态容量与并发执行之间的关系，可以得到本课程的系统主线：
...
本章先规定贯穿全书的请求阶段、状态对象和性能指标。后续 CUDA kernel、PagedAttention、continuous batching 与并行部署章节将分别展开其中一个执行层次。

---

## 1.2 输入表示与生成控制

Token、logits 与 sampling 分别属于输入表示、模型输出和生成控制三个层次。三者在服务接口中连续出现，但不能视作同一对象。[Transformers tokenizer 文档](https://huggingface.co/docs/transformers/main_classes/tokenizer)给出了文本、token 与模型输入字段之间的框架接口；模型相关的具体切分仍由对应 tokenizer 词表和规则决定。

### 1.2.1 文本、Token 与 Token ID
...
### 1.2.3 Greedy、Temperature 与 Sampling
...
Sampling 参数不仅改变输出分布，也可能改变请求能否进入同一个执行批次。[Transformers generation strategies](https://huggingface.co/docs/transformers/main/en/generation_strategies)区分 greedy、sampling、beam search 等生成方法；比较 serving 性能时，应固定 temperature、top-k/top-p、随机数状态和输出上限。

### 1.2.4 EOS、Stop 与 Finish Reason
...
因此，服务端需要在 token 之间重新进入调度，检查停止条件，更新缓存，并决定输出事件的发送时机。一次 API 请求对应的是多次模型执行与状态转换，而不是一次阻塞式模型调用。
...
[DistServe](https://www.usenix.org/system/files/osdi24-zhong-yinmin.pdf)将 prefill 与 decode 的资源特征及对应 SLO 分开建模，并进一步研究两阶段的物理分离。本课程在第一章只采用“分别定义和测量”这一分析边界；是否分离部署需要结合硬件、通信和 workload 另行判断。

## 1.6 KV Cache 的初步容量模型
...
[PagedAttention 论文](https://arxiv.org/abs/2309.06180)以固定大小的物理 KV blocks 和逻辑到物理映射减少连续预留造成的浪费。第 10 章将进一步说明 block table、内部碎片、引用计数和分配失败；本节只用于建立并发容量不能由权重大小直接推出这一结论。
...
### 1.7.2 Inter-Token Latency 与 Time per Output Token
...
不同 benchmark 对 TPOT/ITL 的采样边界可能采用不同约定。[NVIDIA AIPerf 指标参考](https://docs.nvidia.com/aiperf/dev/reference/ai-perf-metrics-reference)将 ITL 定义为首 token 之后的 client-observed decode duration 除以 `output_tokens - 1`，并明确指出 streamed chunk 可能含多个 token。报告指标时必须给出公式、时间戳位置和 token/chunk 计数方式。
...
### 1.7.6 Queueing、Service Time 与 Goodput
...
### 1.8.2 Sequence Budget 与 Token Budget
...
### 1.8.3 Streaming 与 Batching 的职责边界
...
## 1.9 Offline Batching 与 Online Serving 的共享执行内核
...
生产系统的具体类名和进程划分会随版本变化，但请求队列、调度器、模型执行器与 sequence state 之间的职责关系相对稳定。[Orca](https://www.usenix.org/conference/osdi22/presentation/yu)以 iteration-level scheduling 说明生成请求能够在模型迭代边界重新组成 batch；第 9、11 章将把这一机制映射到课程工程。

## 1.10 指标与执行语义的边界

### 1.10.1 Streaming 不等同于模型加速

不一定。Streaming 主要让已经生成的 token 更早可见。即使模型总耗时不变，用户体验也会更好，因为不必等待完整回答。

### 1.10.2 TTFT 不等同于 GPU Prefill 时间

不是。请求在 prefill 前可能已经排队，prefill 后还要 sampling 和发送首 token。要得到纯 GPU prefill 时间，需要在服务端分段测量。

### 1.10.3 GPU 利用率不是独立优化目标

如果为了让 GPU 满载而等待更大的 batch，TTFT 可能变差；如果把太多请求放进系统，p99 和错误率可能失控。利用率是手段，SLO 才是约束。

### 1.10.4 权重容量不能直接换算并发数

权重不是唯一显存消耗。KV cache 随总上下文 token 数增长，临时张量和碎片也需要空间。并发容量必须计算，而不能按权重大小做除法。

### 1.10.5 控制流异步不等同于 GPU 并行
...
- 课程工程：[`engine.py`](../../../course_vllm/engine/engine.py)、[`scheduler.py`](../../../course_vllm/engine/scheduler.py) 与 [`bench_server.py`](../../../course_vllm/benchmarks/bench_server.py)
