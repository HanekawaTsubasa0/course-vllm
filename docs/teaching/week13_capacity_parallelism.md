...
## 9. 背景知识：Expert Parallelism 与 Context Parallelism
...
容量规划的核心是把问题拆成显存、计算、通信和 workload 假设。显存估算要先分清权重、KV cache、临时 activation 和安全余量；吞吐估算要区分 prefill 和 decode；多卡判断要看瓶颈是权重放不下、KV cache 放不下、计算不够，还是通信抵消收益。学完本节后，应能手算一个简化模型的 KV cache 容量，并能解释 TP、PP、EP、CP 分别适合解决什么问题。
