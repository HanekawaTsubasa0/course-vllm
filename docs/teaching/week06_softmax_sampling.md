...
## 5. 背景知识：Greedy decoding 与 sampling
...
softmax 的基本正确性标准是每行概率和接近 1，并且数值结果与同一公式的参考计算在误差范围内一致。

sampling 还要注意随机性。`temperature=0` 通常走 greedy，因此更适合可复现分析；`temperature>0` 会引入随机采样，同一个 prompt 输出不同是正常现象。固定 seed 只能让某一次采样过程可复现，不代表采样本身变成确定性算法。学完本节后，应能解释 logits、softmax、temperature、top-k 和随机数各自影响生成结果的哪一步。
