...
这个文档说明 course LLM serving 工程里每个源码文件的职责。公开运行入口统一为 `--backend reference|course` 与 `--kv-mode dense|paged`；`course_vllm/model/qwen3_backend.py` 仍然保留兼容导出，所以早期 import 不需要修改。
...
| `course_vllm/engine/engine.py` | 推理主入口。选择 `reference` / `course` backend，并在 course backend 内选择 `dense` / `paged` KV mode；实现 `generate`、`generate_stream`、`generate_batch`、`chat`、`chat_stream`，并负责 prefill/decode 循环、采样、停止条件和 KV cache 释放。 |
...
| `course_vllm/engine/kv_cache.py` | 连续 KV cache。用于 `course --kv-mode dense`，直接按层保存 dense K/V 张量，便于理解 KV cache 基本机制。 |
...
| `course_vllm/model/qwen3_continuous_backend.py` | 课程主线的 dense KV runner。封装 tokenizer、chat template、prefill、decode step、batch prefill、batch decode，把 `Qwen3ForCausalLM` 接到 `ContinuousKVCache`。 |
| `course_vllm/model/qwen3_paged_backend.py` | 课程主线的 paged KV runner。继承 dense runner 的公共能力，改用 `BlockManager` 和 `PagedKVCache`，在 decode 阶段通过 block table 走 paged attention。 |
...
| `course_vllm/kernels/cuda_ops.py` | Python wrapper。向上层暴露 `cuda_softmax`、`cuda_rms_norm`、`cuda_rope`、`cuda_matmul`、`cuda_matmul_tiled`、`cuda_dense_attention_prefill`、`cuda_dense_attention_decode`、`cuda_paged_attention_decode`。 |
...
| `course_vllm/benchmarks/capacity_planner.py` | 第十三节容量规划工具。估算 KV blocks、token slots、full-length sequence 上限，以及 TP/PP/EP/CP 的计算量、通信量和显存拆分，并可输出 Markdown 报告。 |
| `course_vllm/benchmarks/system_optimization.py` | 第十二节系统优化实验工具。输出 pinned memory、transfer stream、overlap 和 admission control 的实验配置。 |
| `course_vllm/benchmarks/cache_aware_demo.py` | 第十五节前沿专题 demo。展示 cache-aware ordering 和 paper-to-system 映射。 |
| `course_vllm/benchmarks/grader.py` | 按周运行自动评测。 |
...
| `kernels/course_ops.cu` | CUDA kernel 实现。包含 row-wise softmax、RMSNorm、Qwen RoPE、naive matmul、tiled matmul、dense prefill attention、dense decode attention、paged attention decode。 |
